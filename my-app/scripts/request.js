...
async function main() {
    ...
    // Consumer contract
    const consumerAddress = '0xF2D77eb35cc07AFadFc726b78c9A7754224fEEBc'
    ...
    const verificationBlocks = 3 // Number of blocks to wait for transaction

    // Chainlink Functions request config
    // Chainlink Functions subscription ID
    const subscriptionId = 557
    ...
    const secrets = { apiKey: process.env.OPEN_AI2 }
    ...
    try {
      // Try ethers v6 logs
      console.log("ech")
      requestId = requestTxReceipt.logs[0].args.id
      console.log(`\nRequest ${requestId} initiated`)
    } catch (e) {
      if (e instanceof TypeError) {
        // Try ethers v5 events
        requestId = requestTxReceipt.events[0].args.id
